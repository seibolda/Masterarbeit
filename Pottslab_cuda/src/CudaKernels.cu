...
__device__ void doPottsStepKernel(float* arrayToUpdate, float* weights, float gamma, uint32_t rowCol, uint32_t n, uint32_t widthOrHeight, uint32_t nc) {
    uint32_t y = rowCol;
    ...
    for(uint32_t j = 0; j < n; j++) {
        wTemp = weights[j + y*n];
        m[j + 1] = arrayToUpdate[j + y*n] * wTemp + m[j];
        m[j + 1 + n+1] = arrayToUpdate[j + y*n + n*h] * wTemp + m[j + n+1];
        m[j + 1 + 2*(n+1)] = arrayToUpdate[j + y*n + 2*n*h] * wTemp + m[j + 2*(n+1)];
        s[j + 1] = normQuadKernel(arrayToUpdate, j+y*n, n*h) * wTemp + s[j];
        w[j + 1] = w[j] + wTemp;
    }
    ...
    while(r > 0) {
        ...
        for(uint32_t j = l; j < r; j++) {
            for(uint32_t k = 0; k < nc; k++) {
                arrayToUpdate[j + y*n + k*n*h] = mu[k];
            }
        }
        r = l;
        if (r < 1) break;
...
__global__ void applyHorizontalPottsSolverKernel(float* u, float* weights, float gamma, uint32_t w, uint32_t h, uint32_t nc) {
    ...
    if(y < h) {
        doPottsStepKernel(u, weights, gamma, y, w, h, nc);
    }
}
...
__global__ void applyVerticalPottsSolverKernel(float* v, float* weights, float gamma, uint32_t w, uint32_t h, uint32_t nc) {
    ...
    if(x < w) {
        doPottsStepKernel(v, weights, gamma, x, h, w, nc);
    }
}
